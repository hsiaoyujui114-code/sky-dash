...
export default function Game() {
  ...
  const [multiplayerWinner, setMultiplayerWinner] = useState<string | null>(
    null,
  );
  ...
  const currentSeedRef = useRef<number>(Math.floor(Math.random() * 233280));
  ...
  const getRank = (currentScore: number) => {
    ...
    if (nextRank !== currentRank) {
      progress =
        ((currentScore - currentRank.threshold) /
  ...
  const sendGameStatsToGoogleAppsScript = useCallback(
  ...
  const createRoom = () => {
    ...
    const shortCode = String(Math.floor(Math.random() * 10000)).padStart(
      4,
      "0",
    );
  ...
  const startRaceMultiplayer = () => {
    ...
    const gameSeed = Math.floor(Math.random() * 233280);
    setRoomState(newState);
    roomStateRef.current = newState;
  ...
  const update = useCallback(() => {
    ...
    const currentSpeed =
      (speedRef.current + (pUps.boost > 0 ? 10 : 0)) *
      (pUps.slow > 0 ? 0.5 : 1);
    ...
    if (
      ...
      frameCountRef.current %
        Math.max(
          15,
          config.obstacleFrequency - Math.floor(speedRef.current),
        ) ===
        0
    ) {
        ...
        if (item.type === "coin") {
          ...
          if (Math.random() < 0.6 && yHistoryRef.current.length > 0) {
            const randomIndex = Math.floor(
              Math.random() * yHistoryRef.current.length,
            );
  ...
  const draw = useCallback(() => {
    ...
    const currentSpeed =
      (speedRef.current + (powerupsRef.current.boost > 0 ? 10 : 0)) *
      (powerupsRef.current.slow > 0 ? 0.5 : 1);
    if (currentLevel === "dungeon") {
    ...
    if (isMultiplayer || isDungeon) {
      ...
      const smoothScore =
        scoreRef.current +
        ((frameCountRef.current % 10) / 10) *
          (powerupsRef.current.doubleScore > 0 ? 2 : 1);
      const finishLineX =
        playerRef.current.x + (goal - smoothScore) * (speedRef.current * 10);
      ...
      if (finishLineX > -100 && finishLineX < CANVAS_WIDTH + 100) {
        ...
        for (let y = 0; y < CANVAS_HEIGHT; y += squareSize) {
          ctx.fillStyle =
            Math.floor(y / squareSize) % 2 === 0 ? "#fff" : "#000";
          ctx.fillRect(finishLineX, y, squareSize, squareSize);
          ctx.fillStyle =
            Math.floor(y / squareSize) % 2 === 0 ? "#000" : "#fff";
    ...
    if (isMultiplayerRef.current && roomStateRef.current) {
      Object.values(otherPlayersRef.current).forEach((p: any) => {
        ...
        if (relativeX > -50 && relativeX < CANVAS_WIDTH + 50) {
          ctx.save();
          ctx.translate(
            relativeX + playerRef.current.width / 2,
            p.y + playerRef.current.height / 2,
          );
  ...
  useEffect(() => {
    ...
    if (gameState === "start") {
      fetchPlayerCount().then(setTotalPlayers);
    }
  ...
  const renderShipPreview = (shipId: ShipType, isSelected: boolean) => {
    ...
    return (
      ...
      >
        ...
        <span
          className={`font-bold text-sm ${isSelected ? "text-emerald-400" : "text-slate-300"}`}
        >
          {ship.name}
        </span>
        {isSelected && (
    ...
    <div className="relative w-full max-w-4xl aspect-[4/3] bg-slate-900 rounded-xl overflow-hidden shadow-2xl ring-4 ring-slate-800">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <div className="bg-slate-800/50 backdrop-blur px-3 py-1 rounded-lg border border-slate-700/50 flex items-center gap-2">
                  <span
                    className={`font-mono text-sm font-bold ${LEVELS[currentLevel].color}`}
                  >
                ...
                <div className="bg-slate-800/50 backdrop-blur px-3 py-1 rounded-lg border border-slate-700/50 flex items-center gap-2">
                  <span
                    className={`font-mono text-sm font-bold ${currentRank.color}`}
                  >
                    {currentRank.name}
                  </span>
                </div>
                ...
                >
                  <span
                    className={`font-mono text-xs font-bold ${copiedWorldCode ? "text-green-400" : "text-cyan-400"}`}
                  >
                    {copiedWorldCode
                      ? "COPIED!"
              ...
              {powerups.doubleScore > 0 && (
                <div className="bg-yellow-500/20 px-3 py-2 rounded-lg border border-yellow-500/50 flex items-center gap-2 animate-pulse">
                  ...
                  <span className="text-yellow-300 font-mono text-sm">
                    {Math.ceil(powerups.doubleScore / 60)}s
                  </span>
                </div>
              )}
              {powerups.weapon > 0 && (
                <div className="bg-green-500/20 px-3 py-2 rounded-lg border border-green-500/50 flex items-center gap-2 animate-pulse">
                  ...
                  <span className="text-green-300 font-mono text-sm">
                    {Math.ceil(powerups.weapon / 60)}s
                  </span>
                </div>
              )}
              {powerups.star > 0 && (
      ...
      {gameState === "start" && (
        <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm flex flex-col items-center justify-center">
          <h1 className="mt-[64px] text-6xl font-black text-transparent bg-clip-text bg-gradient-to-br from-emerald-400 to-cyan-500 mb-3 tracking-tight">
            SKY DASH
          </h1>
          {totalPlayers > 0 && (
                ...
                <div className="space-y-6 text-slate-300 leading-relaxed pointer-events-auto">
                  <section>
                    ...
                    <ul className="list-disc pl-5 space-y-2">
                      <li>
                        <strong>電腦版：</strong>按住{" "}
                        <code>空白鍵 (Space)</code> 或 <code>滑鼠左鍵</code>{" "}
                        讓飛船上升，放開則會依重力下降。
                      </li>
                      <li>
                        <strong>手機/平板版：</strong>
                        點擊並按住螢幕任何地方讓飛船上升，放開則會下降。
                      </li>
                    </ul>
                  </section>
                  ...
                  <section>
                    ...
                    <p>
                      避開所有紅色的障礙物，盡可能飛得更遠來獲得高分！碰到障礙物遊戲就會結束。
                    </p>
                  </section>
                  ...
                  <section>
                    ...
                    <ul className="space-y-3">
                      <li className="flex items-center gap-3">
                        ...
                        <div>
                          <strong>護盾 (Shield)：</strong> 獲得無敵狀態 10
                          秒，可抵擋一次撞擊。
                        </div>
                      </li>
                      <li className="flex items-center gap-3">
                        ...
                        <div>
                          <strong>衝刺 (Boost)：</strong> 獲得極速衝刺 5
                          秒，期間無敵且會摧毀障礙物。
                        </div>
                      </li>
                      <li className="flex items-center gap-3">
                        ...
                        <div>
                          <strong>雙倍分數 (Double Score)：</strong> 10
                          秒內獲得的分數變為兩倍。
                        </div>
                      </li>
                      <li className="flex items-center gap-3">
                        ...
                        <div>
                          <strong>武器 (Weapon)：</strong> 獲得發射子彈的能力 10
                          秒，可擊碎前方障礙物。
                        </div>
                      </li>
                      <li className="flex items-center gap-3">
                        ...
                        <div>
                          <strong>無敵星 (Star)：</strong> 獲得完全無敵狀態 10
                          秒，閃爍發光並摧毀所有碰到的障礙物。
                        </div>
                      </li>
                      <li className="flex items-center gap-3">
                      ...
                      <li className="flex items-center gap-3">
                        ...
                        <div>
                          <strong>獎盃 (Trophy)：</strong> 獲得大量分數 (1000分)
                        </div>
                      </li>
                    </ul>
                  </section>
                  ...
                  <section>
                    ...
                    <ul className="list-disc pl-5 space-y-2">
                      <li>
                        <strong>單人模式：</strong>
                        分為簡單(Easy)、普通(Medium)、困難(Hard)、專家(Expert)、瘋狂(Insane)以及地城(Dungeon)模式。難度越高，速度越快、障礙物越密集。
                      </li>
                      <li>
                        <strong>多人模式 (Multiplayer)：</strong>
                        可以建立房間並邀請朋友一起競速！首先到達設定終點線的玩家獲勝。
                      </li>
                      <li>
                        <strong>機庫 (Hangar)：</strong>可以選擇不同外觀的飛船。
                      </li>
                    </ul>
                  </section>
                </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-12 pointer-events-none">
            ...
            <div className="flex items-center gap-4 bg-slate-800/50 p-3 rounded-xl">
              ...
              <div>
                ...
                <div className="text-slate-400 text-sm">
                  Invincible & destroys all for 10s
                </div>
              </div>
            </div>
            <div className="flex items-center gap-4 bg-slate-800/50 p-3 rounded-xl">
            ...
            <div className="flex items-center gap-4 bg-slate-800/50 p-3 rounded-xl">
              ...
              <div>
                ...
                <div className="text-slate-400 text-sm">
                  Fires a missile, +200 pts on hit
                </div>
              </div>
            </div>
            <div className="flex items-center gap-4 bg-slate-800/50 p-3 rounded-xl">
      ...
      {gameState === "level_select" && (
        <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-md overflow-y-auto flex flex-col items-center p-8">
          <div className="flex flex-col items-center w-full">
            <h2 className="text-4xl font-black text-white mb-8 tracking-tight sticky top-0 z-10">
              SELECT LEVEL
            </h2>

            <div className="w-full max-w-2xl mb-8 bg-slate-800 p-4 rounded-xl border border-slate-700">
              <label className="block text-sm font-bold text-slate-300 mb-2">
                Enter World Code (Optional)
              </label>
              <div className="flex gap-2">
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    setWorldCodeInput(e.target.value.toUpperCase().slice(0, 6))
                  }
              ...
              <p className="text-xs text-slate-400 mt-2">
                Enter a 6-character world code to replay a world
              </p>
            </div>
      ...
      {gameState === "victory" && (
        <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-md flex flex-col items-center justify-center pointer-events-none">
          <h2 className="text-5xl font-black text-emerald-400 mb-4 tracking-tight drop-shadow-[0_0_15px_rgba(16,185,129,0.5)]">
            DUNGEON CLEARED!
          </h2>
          <div className="flex flex-col items-center mb-6">
            <Trophy
              className={`w-16 h-16 mb-2 text-yellow-400 drop-shadow-lg`}
            />
            ...
            <div className="p-6 flex flex-col gap-4">
              {!roomState ? (
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1 flex gap-2">
                      ...
                      <button
                        ...
                        disabled={
                          !joinRoomIdInput || !playerName || isLobbyLoading
                        }
                ...
                <>
                  <div className="bg-slate-900 rounded-lg p-4 border border-slate-700">
                    ...
                    <div className="flex flex-col gap-2">
                      {Object.values(roomState.players).map((p: any) => (
                        <div
                          key={p.id}
                          className="flex items-center gap-3 bg-slate-800 p-2 rounded-lg"
                        >
                          <div
                          ...
                          <span className="text-white font-bold">
                            {p.name}{" "}
                            {p.id === myPlayerIdRef.current ? "(You)" : ""}
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-slate-900 rounded-lg p-4 border border-slate-700">
                    <h3 className="text-slate-400 text-sm font-bold mb-3">
                      Race Distance
                    </h3>
                    <select
        ...
        <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start pointer-events-none">
          ...
          <div className="flex flex-col gap-2 items-end">
            <div className="bg-slate-800/80 backdrop-blur px-4 py-3 rounded-lg border border-slate-700 flex flex-col gap-2 min-w-[280px]">
              ...
              {[
                myPlayerIdRef.current,
                ...Object.keys(otherPlayersRef.current).filter(
                  (id) => id !== myPlayerIdRef.current,
                ),
              ].map((playerId) => {
                const p =
                  playerId === myPlayerIdRef.current
                  ...
                  <div key={p.id} className="flex flex-col gap-1">
                    <div className="flex justify-between items-center gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        ></div>
                        <span
                          className={`text-xs font-bold truncate ${playerId === myPlayerIdRef.current ? "text-emerald-400" : "text-white"}`}
                        >
                          {p.name}
                          {playerId === myPlayerIdRef.current ? " (You)" : ""}
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-slate-500 text-xs">
                          {Math.floor((p.progress / multiplayerGoal) * 100)}%
                        </span>
                      </div>
                    </div>
      ...
      {gameState === "multiplayer_gameover" && (
        <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-md flex flex-col items-center justify-center pointer-events-none">
          <h2 className="text-5xl font-black text-yellow-500 mb-2 tracking-tight">
            RACE FINISHED!
          </h2>
          <p className="text-xl text-white mb-8">
            {multiplayerWinner === myPlayerIdRef.current
              ? "You won the race!"
              : `${otherPlayersRef.current[multiplayerWinner || ""]?.name || "Someone"} won the race!`}
          </p>

          <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 mb-8 flex flex-col gap-4 min-w-[300px]">
            <h3 className="text-slate-400 font-bold text-sm uppercase tracking-wider border-b border-slate-700 pb-2">
              Final Standings
            </h3>
            {Object.values(otherPlayersRef.current)
              .sort(
                (a: any, b: any) =>
                  b.progress +
                  b.trophies * 1000 -
                  (a.progress + a.trophies * 1000),
              )
              .map((p: any, index: number) => (
                <div
                  key={p.id}
                  className={`flex justify-between items-center p-3 rounded-lg ${p.id === multiplayerWinner ? "bg-yellow-500/20 border border-yellow-500/50" : "bg-slate-900/50"}`}
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`font-black ${index === 0 ? "text-yellow-500" : index === 1 ? "text-slate-300" : index === 2 ? "text-orange-600" : "text-slate-500"}`}
                    >
                      #{index + 1}
                    </span>
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-yellow-500 text-xs font-mono flex items-center gap-1">
                      <Trophy className="w-3 h-3" /> {p.trophies}
                    </span>
                  </div>
                </div>
              ))}
